...
class Google_client_api {
	...
	public function youtube_upload($video="linkedin.mp4",$title="tvn rahul youtube api v3",$desc="tvn rahul youtube api v3 for php",$tags=["rahultvn","youtubeapi3"],$privacy_status="public") {
		...
		$key = file_get_contents('./asset/the_key.txt');
		//print_r($key->access_token);die;

		$client = new Google_Client();
		$client->setApplicationName("abcd");
		$client->setClientId($OAUTH2_CLIENT_ID);
		$client->setAccessType('offline');
		//print_r($client->setAccessToken($key));die;
		...
		$refreshToken = "1/J15wUS3d28rznXSDvnv6BZ5JlO0LF9MkY-qmFX9I0Go";
		...
		if (isset($_SESSION['access_token']) && $_SESSION['access_token']) {
			...
			/*if($client->isAccessTokenExpired()) {
	            ...
	            file_put_contents('./asset/the_key.txt', $client->getAccessToken());
	        }*/
	        $youtube = new Google_Service_YouTube($client);
		  try{
			// REPLACE this value with the path to the file you are uploading.
			realpath(APPPATH . 'asset/videos/Whatsapp.mp4');
			$videoPath = $video;
